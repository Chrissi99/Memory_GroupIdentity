...
class C(BaseConstants):
    ...
    ROWS_EFFORT = 8
    COLUMNS_EFFORT = 10
    MATRICES = [np.random.choice([0, 1], size=(8, 10)).tolist() for _ in range(10)]
    # SOLUTIONS = [[row.count(0) for row in matrix] for matrix in MATRICES]
    SOLUTIONS = [sum(cell == 0 for row in matrix for cell in row) for matrix in MATRICES]
    name_in_url = "counting"
...
class Player(BasePlayer):
    ...
    #score_effort = models.IntegerField(
     #   initial=0,
      #  blank=True)
    #answers_effort = models.LongStringField()
    iteration_effort = models.IntegerField(initial=0)
    num_trials_effort = models.IntegerField(initial=0)
    num_correct_effort = models.IntegerField(initial=0)
    num_failed_effort = models.IntegerField(initial=0)
...
class LogicTask1(Page):
    ...
    def before_next_page(player: Player, timeout_happened):
        ...
        # participant.score_logic = player.score_logic
        #if player.round_number == C.NUM_ROUNDS:
         #   print('logic score:', participant.score_logic)
    @staticmethod
    def is_displayed(player: Player):
        participant = player.participant
...
class InstructionsEffort(Page):
    @staticmethod
    def is_displayed(player: Player):
        participant = player.participant
        return player.round_number == participant.task_order['effort']



class CountingTask(Page):
    timeout_seconds = 60 * 2  # changed from 60*50
    ...
    def before_next_page(player: Player, timeout_happened):
        ...
        print('score real effort:', player.num_correct_effort)
        participant = player.participant
        participant.score_effort = player.num_correct_effort
        print('score effort:', participant.score_effort)
    ...
    def before_next_page(player: Player, timeout_happened):
        ...
        for i in range(len(player.rolls)):
            if player.rolls[i] == "6":
                ...
                player.score_luck += 1
        print('score_luck:', player.score_luck)
        participant = player.participant
...
page_sequence = page_sequence = [InstructionsTask, InstructionsLogic, LogicTask1, LogicTask2, LogicTask3, LogicTask4, LogicTask5, LogicTask6, # LogicTask7, LogicTask8,
                                 InstructionsEffort, CountingTask, InstructionsDice, DiceTask1]
#[InstructionsTask, InstructionsLogic, LogicTask1, LogicTask2, LogicTask3, InstructionsEffort, EffortTask1, InstructionsDice, DiceTask1]
    #[InstructionsTask, InstructionsLogic, LogicTask1, LogicTask2, LogicTask3, InstructionsEffort, EffortTask1, InstructionsDice, DiceTask1]
